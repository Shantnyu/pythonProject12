...
def dns_query(type, name, server):
    ...
    server_address = (server, 53) # DNS standard port number
    ...
    # Pack the DNS header fields into a binary format for the query
    header = struct.pack('!HHHHHH', ID, QR << 15 | OPCODE << 11 | AA << 10 | TC << 9 | RD << 8 | RA << 7 | Z << 4 | RCODE, QDCOUNT, ANCOUNT, NSCOUNT, ARCOUNT)

    # Split the domain name into its parts and encode it
    ...
    # Determine the QTYPE based on the type of query
    if type == 'A':
        qtype = 1  # Type A Resource Record value
    elif type == 'AAAA':
        qtype = 28  # Type AAAA Resource Record value
    else:
        raise ValueError('Invalid type')

    qclass = 1  # Class IN

    # Combine the QNAME, QTYPE, and QCLASS to form the question part of the query
    question = qname_encoded + struct.pack('!HH', qtype, qclass)

    # Send the query to the server, which includes the header and the question
    ...
    # The response question section should match the question we sent
    ...
    # Go through all the answers provided in the response
    for _ in range(ANCOUNT):
        # Parse the name from the answer
        name_parts = []
        while True:
...
# This function parses the domain name from a pointer in the message
